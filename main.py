...
class MatexCompiler:
    ...
    # Warning: this function is out of date
    def _parse_v1(self, **kwargs) -> bool:

        for head, tail in self._input:

            if tail is None:
                return self._error('there should be a parameter after the command')
            ...
            try:
                ...
                return self._error('unmatched `%`')
            except Exception as error:
                return self._error(f'unknown variable `{error.args}`')
            ...
            if head == 'DEFINE':
                ...
                self._print(f'unexpected tag `{head}`')
